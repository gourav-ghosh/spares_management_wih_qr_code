...
class SparesController extends Controller
{
    ...
    public function spare_dashboard(Request $request)
    {
        if (Auth::check()) {
            ...
            return redirect('/login')->back()->withErrors(['error' => ['Login to access spares dashboard']]);
        }
    }
    ...
    public function spare_dashboard_storage(Request $request, $place)
    {
        if (Auth::check()) {
            ...
            $spares = Spares::with(['medias'])->where('spare_storage', $place)->orderBy('updated_at')->paginate($per_page);
