...
>
    ...
    <div class="scrollmenu">
        <a
            id="workshop"
            type="button"
            href="/dashboard/spares/workshop"
            name="answer"
            id="btn0"
            class="notifier new"
        >
        ...
        <a
            id="roll_stand"
            type="button"
            href="/dashboard/spares/roll_stand"
            name="answer"
            id="btn0"
            class="notifier new"
        >
        ...
        <a
            id="mcl_exit"
            type="button"
            href="/dashboard/spares/mcl_exit"
            name="answer"
            id="btn1"
            class="notifier new"
        >
    ...
    <div class="container text-center">
        ...
        @if(count($spares)>0) @foreach($spares as $spare)
                        ...
                        <div style="word-wrap: normal; width: 48%">
                            <div style="word-wrap: break-word; margin: 5px 0px">
                                <span style="font-weight: 600; font-size: 18px">
                                    Id: @if($spare->spare_id){{$spare->spare_id}}
                                    @else Not Available @endif</span
                                >
                            </div>
                            <div style="word-wrap: break-word; margin: 5px 0px">
                                <span style="font-weight: 500; font-size: 16px">
                                    Name: @if($spare->spare_name){{$spare->spare_name}}
                                    @else Not Available @endif</span
                                >
                            </div>
                            <div style="word-wrap: break-word; margin: 5px 0px">
                                <span style="font-weight: 500; font-size: 16px">
                                    Department: @if($spare->department &&
                                    $spare->department == 'mcl') MCL
                                    @elseif($spare->department &&
                                    $spare->department == 'ccl') CCL
                                    @elseif($spare->department &&
                                    $spare->department ==
                            ...
                            <div style="word-wrap: break-word; margin: 5px 0px">
                                <span style="font-weight: 500; font-size: 16px">
                                    Type: @if($spare->spare_type){{$spare->spare_type}}
                                    @else Not Available @endif</span
                                >
                            </div>
                            <div style="word-wrap: break-word; margin: 5px 0px">
                                <span style="font-weight: 500; font-size: 16px">
                                    Storage: @if($spare->spare_storage &&
                                    $spare->spare_storage == 'roll_stand') Roll
                                    Stand @elseif($spare->spare_storage &&
                                    $spare->spare_storage == 'workshop')
                                    Workshop Area @elseif($spare->spare_storage
                                    && $spare->spare_storage == 'spm') SPM Area
                                    @elseif($spare->spare_storage &&
                                    $spare->spare_storage == 'os_mcl') Oil
                                    Storage MCL @elseif($spare->spare_storage &&
                                    $spare->spare_storage == 'os_ccl') Oil
                                    Storage CCL @elseif($spare->spare_storage &&
                                    $spare->spare_storage == 'mcl_exit') MCL
                                    Exit @elseif($spare->spare_storage &&
                                    $spare->spare_storage == 'jk_bay') JK Bay
                                    Area @elseif($spare->spare_storage &&
                                    $spare->spare_storage == 'de_bay') CCL DE
                                    Bay area @elseif($spare->spare_storage &&
                                    $spare->spare_storage == 'cd_bay') MCL CD
                                    Bay area @else Not Available @endif</span
                                >
                            </div>
                        </div>
                    </div>
...
<script>
    function rows_per_page() {
        ...
        window.location.href = `/dashboard/spares?${parseResult}&rows_per_page=${per_page}`;
    }
</script>
