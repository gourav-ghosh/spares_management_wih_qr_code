...
<div class="container-fluid" style="padding-right: var(--bs-gutter-x,0rem); padding-left: var(--bs-gutter-x,0rem); margin-top: -50px;">
    <div class="main_container">
        ...
        <div style="margin-bottom: 15px; font-size: 20px; margin-left: 0px; display: flex; justify-content: space-between; gap: 10px; margin-right: 10px;">
            @if(Auth::check())
            <div>
                <a href="/dashboard/maintenance" style="text-decoration: none; margin-right: 10px;">
        ...
        <div style="margin-bottom: 15px; font-size: 30px; font-weight: 600; text-align: center; word-wrap: break-word;">
            <u> ID: @if($maintenance_detail->machine && $maintenance_detail->machine->machine_id){{$maintenance_detail->machine->machine_id}} @elseif($maintenance_detail->spare && $maintenance_detail->spare->spare_id){{$maintenance_detail->spare->spare_id}} @endif </u>
        </div>
        @php 
            $media_display=null;
            if($maintenance_detail->machine_id)
            {
                ...
                if(!$media_display)
                {
                    $media_display = $maintenance_detail->machine->medias[0];
                }
            }
            elseif($maintenance_detail->spare_id)
            {
                $alt_name = $maintenance_detail->spare->spare_name;
                ...
                if(!$media_display)
                {
                    $media_display = $maintenance_detail->spare->medias[0];
                }
            }
        @endphp
        <div style="display: flex; justify-content: center; align-items: center; width: 95%; aspect-ratio: 1/1; margin-left: 2%;">
            <img src="{{$media_display->path}}" alt="{{$alt_name}}" srcset="" width="100%" height="100%">
        </div>
        ...
        <div class="accordion-item" style="margin: 10px 0px;">
            <h2 class="accordion-header" id="flush-headingOne">
                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                    ...
                    @if($maintenance_detail->machine_id)
                    @php($maintenance_medias = $maintenance_detail->machine->medias)
                    @elseif($maintenance_detail->spare_id)
                    @php($maintenance_medias = $maintenance_detail->spare->medias)
                    @endif
                    @if(count($maintenance_medias) != 0)
                    @foreach($maintenance_medias as $media)
                    @if(($media->for_status == "defect" || $media->for_status == "maintenance") && ($media->media_type == "image" || $media->media_type == "video"))
                    ...
                    <section class="gallery">
                        <div class="container-lg">
                            <div class="row gy-2 row-cols-2 row-cols-sm-2 row-cols-md-3">
                                ...
                                @if(count($maintenance_medias) != 0)
                                @foreach($maintenance_medias as $media)
                                @if(($media->for_status == "defect" || $media->for_status == "maintenance") && $media->media_type == "image")
                                @php($detail_image++)
                                ...
                                @if(($media->for_status == "defect" || $media->for_status == "maintenance") && $media->media_type == "video")
                                @php($detail_image++)
        ...
        <div style="font-size: 18px; letter-spacing: 2px; margin-left: 20px; word-spacing: 10px; margin-top: 10px;">
            <b>Name: </b> @if($maintenance_detail->machine && $maintenance_detail->machine->machine_name){{$maintenance_detail->machine->machine_name}} @elseif($maintenance_detail->spare && $maintenance_detail->spare->spare_name){{$maintenance_detail->spare->spare_name}} @else N.A. @endif
        </div>
        <div style="font-size: 18px; letter-spacing: 2px; margin-left: 20px; word-spacing: 10px; margin-top: 5px;">
            <b>Type: </b> @if($maintenance_detail->machine && $maintenance_detail->machine->machine_type){{$maintenance_detail->machine->machine_type}} @elseif($maintenance_detail->spare && $maintenance_detail->spare->spare_type){{$maintenance_detail->spare->spare_type}} @else N.A. @endif
        </div>
        @if($maintenance_detail->machine && $maintenance_detail->machine->department)
            @php($department = $maintenance_detail->machine->department)
        @elseif($maintenance_detail->spare && $maintenance_detail->spare->department)
            @php($department = $maintenance_detail->spare->department)
        @endif
        
                            
        <div style="font-size: 18px; letter-spacing: 2px; margin-left: 20px; word-spacing: 10px; margin-top: 5px;">
            <b>Department: </b> 
            @if($department && $department == 'mcl') MCL 
            @elseif($department && $department == 'ccl') CCL
            @elseif($department && $department == 'mechanical_maintenance') Mechanical Maintenance
            @else N.A. @endif
        </div>
        <div style="font-size: 18px; letter-spacing: 2px; margin-left: 20px; word-spacing: 10px; margin-top: 5px;">
            <b>Defect: </b> <span style="color: #f72c1d;"> @if($maintenance_detail->defect)
                {{$maintenance_detail->defect}}
                @else Not Available @endif </span>
        </div>
        <div style="font-size: 18px; letter-spacing: 2px; margin-left: 20px; word-spacing: 10px; margin-top: 5px;">
            <b>Issue Raising Date: </b> {{$maintenance_detail->created_at->format('d-m-y')}}
        </div>
        <div style="font-size: 18px; letter-spacing: 2px; margin-left: 20px; word-spacing: 10px; margin-top: 5px;">
            <b>Issue Raiseing Time: </b> {{$maintenance_detail->created_at->format('h:i A')}}
        </div>
        <div style="font-size: 18px; letter-spacing: 2px; margin-left: 20px; word-spacing: 10px; margin-top: 5px;">
            <b>Maintenance Completed: </b> @if($maintenance_detail->maintenance_completed) {{Carbon\Carbon::parse($maintenance_detail->maintenance_completed)->format('d-m-y  h:i A')}} @else Not resolved Yet @endif
        </div>
            ...
            <a href="/machine/{{$maintenance_detail->machine_id}}">
                <div style="display: flex; justify-content: left; gap: 0px 10px; margin: 10px 10px; border-radius: 5px; background-color: #ecebe9; padding: 10px; cursor: pointer;">
                    ...
                    <div style="display: flex; justify-content: center; align-items: center; width: 80px; aspect-ratio: 1/1; margin-left: 2%;">
                        <img src="{{$maintenance_detail->machine->medias[0]->thumbnail_path}}" alt="{{$maintenance_detail->machine->machine_name}}" srcset="" width="100%" height="100%">
                    </div>
        ...
        @if($maintenance_detail->spare_id)
        <div style="font-size: 24px; font-weight: 600; margin-left: 3%; margin-top: 20px;">
            <u> Spare details</u>
        </div>
        <div style="margin: 15px 5px;">
            <a href="/spare/{{$maintenance_detail->spare_id}}">
                <div style="display: flex; justify-content: left; gap: 0px 10px; margin: 10px 10px; border-radius: 5px; background-color: #ecebe9; padding: 10px; cursor: pointer;">
                    ...
                    <div style="display: flex; justify-content: center; align-items: center; width: 80px; aspect-ratio: 1/1; margin-left: 2%;">
                        <img src="{{$maintenance_detail->spare->medias[0]->thumbnail_path}}" alt="{{$maintenance_detail->spare->spare_name}}" srcset="" width="100%" height="100%">
                    </div>
                    <div style="display: flex; align-items: center; width: auto; font-size: 18px; font-weight: 600; text-align: justify;">
        ...
        @if(Auth::check() && $maintenance_detail->maintenance_completed == null)
        <div>
            <button data-bs-toggle="modal" data-bs-target="#update_maintenance"
                onclick="show_update_maintenance();"
                style="margin: 30px auto 0px auto; display: flex; justify-content: center; align-items: center; background: linear-gradient(90deg, #01EAFE 0%, #1F7FD9 100%, rgba(0, 0, 0, 1) 100%); font-size: 18px;border: none; min-height: 40px; font-weight: bold;   color: white;border-radius: 5px; text-align: center;letter-spacing: 0.15em;">
                <span>Complete</span>
            </button>
            <div style="" class="modal fade" id="update_maintenance"
                data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-scrollable">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="staticBackdropLabel">Update Maintenance Details</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"
                                onclick="clear_update_form();"></button>
                        </div>
                        <div class="modal-body" id="update_maintenance_modal">
