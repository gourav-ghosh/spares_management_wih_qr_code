...
<div class="add_spare_bg" style="display: flex; justify-content: space-around;">
        ...
        <div>
            <form action="/add_maintenance" method="post" enctype="multipart/form-data">
                ...
                <div style="margin-top: -30px;">
                    <input type="radio" name="product" id="machine" value="machine"
                    style="visibility: hidden;"  required>
                    <input type="radio" name="product" id="spare" value="spare"
                    style="visibility: hidden;"  required>
                </div>
                ...
                <div style="display: flex; justify-content: space-between; margin-top: 10px; margin-right: 10%;">
                    <div id="machine_lable"
                        style="width: 47%; height: 40px; background-color: #9D9D9D; text-align: center; border-radius: 5px; padding-top: 5px;">
                        <label for="machine" style="width: 100%; font-size: 20px; font-weight: bold; color: #F7F6F5;">
                            Machine
                        </label>
                    </div>
                    <div id="spare_lable"
                        style="width: 47%; height: 40px; background-color: #9D9D9D; text-align: center; border-radius: 5px; padding-top: 5px;">
                        <label for="spare"
                            style="width: 100%; font-size: 20px; font-weight: bold; color: #F7F6F5;">
                            Spare
                        </label>
                    </div>
                </div>
                    
                <div id="machine_div" hidden>
                    <div id="machine_div"><span id="customer_style">Machine<span
                        style="color: #F72C1D; padding-left: 1px;">*</span></span>
                    </div>
                    <select name="machine_id" id="machine1" class="machine_select" required>
                ...
                <div id="spare_div" hidden>
                    <div><span id="customer_style">Spare<span
                        style="color: #F72C1D; padding-left: 1px;">*</span></span>
                    </div>
                    <select name="spare_id" id="spare1" class="spare_select" required>
                        <option value="" disabled selected> Select Spare                </option>
                        @if($spares)
                ...
                <div><span id="customer_style">Defect<span style="color: #F72C1D; padding-left: 1px;">*</span></span>
                </div>
                <input id="defect" name="defect" type="text"
                    placeholder="Enter Defect" required>
...
document.getElementById("machine").addEventListener('click', function() {
    document.getElementById("machine_lable").style.background = 'linear-gradient(90deg, #1F7FD9 0%, #01EAFE 100%, rgba(0, 0, 0, 1) 100%)';
    document.getElementById("spare_lable").style.background = '#9D9D9D';
    machine_selected();
    select_function();
});
document.getElementById("spare").addEventListener('click', function() {
    document.getElementById("spare_lable").style.background = 'linear-gradient(90deg, #1F7FD9 0%, #01EAFE 100%, rgba(0, 0, 0, 1) 100%)';
    document.getElementById("machine_lable").style.background = '#9D9D9D';
    spare_selected();
    select_function();
});
...
<script>
    function machine_selected()
    {
        document.getElementById('machine_div').hidden = false;
        document.getElementById('machine1').setAttribute("required", "required");
        document.getElementById('spare_div').hidden = true;
        document.getElementById('spare1').removeAttribute("required");
        document.getElementById('blank_space').hidden = true;
    }
    
    function spare_selected()
    {
        document.getElementById('machine_div').hidden = true;
        document.getElementById('machine1').removeAttribute("required");
        document.getElementById('spare_div').hidden = false;
        document.getElementById('spare1').setAttribute("required", "required");
    }
</script>
<script>
...
function select_function()
{
    ...
    var $element2 = $('.spare_select');
    if ($element2.length) {
      if ($element2.is(':visible')) 
      {
        $(document).ready(function() {
            $('.spare_select').selectize({
                sortField: 'text'
            });
        });
      }
