        ...
        <div style="margin-bottom:20px;" class="maindiv">
            <form action="/maintenance/{{$maintenance->id}}">
                <button type="submit" class="button">
                    ...
                    <div class="status">
                        <p class="card-text text" style="color:white;font-weight:bold; font-size:15px; width:90px;">
                            Pending
                        </p>
                    </div>
                    @else
                    ...
                    <div style="display: flex; justify-content: left; gap: 4%; flex-wrap: wrap; align-items: safe ;">
                        ...
                        if($maintenance->machine_id)
                        {
                            ...
                            if(!$media_display)
                            {
                                $media_display = $maintenance->machine->medias[0];
                            }
                        }
                        elseif($maintenance->spare_id)
                        {
                            $alt_name = $maintenance->spare->spare_name;
                            ...
                            if(!$media_display)
                            {
                                $media_display = $maintenance->spare->medias[0];
                            }
                        }
                        @endphp
                        <div style="width: 45%;">
                            <img style="border-radius: 6px;" src="@if($media_display){{$media_display->thumbnail_path}}@endif" alt="{{$alt_name}} image" srcset="" width="95%">
                        </div>
                        <div style="word-wrap: normal; width: 48%;">
                            <div style="word-wrap: break-word; margin: 5px 0px;">
                                <span style="font-weight: 600; font-size: 18px;"> Id: @if($maintenance->machine && $maintenance->machine->machine_id){{$maintenance->machine->machine_id}} @elseif($maintenance->spare && $maintenance->spare->spare_id){{$maintenance->spare->spare_id}} @else Not Available @endif</span>
                            </div>
                            <div style="word-wrap: break-word; margin: 5px 0px;">
                                <span style="font-weight: 500; font-size: 16px; "> Name: @if($maintenance->machine && $maintenance->machine->machine_name){{$maintenance->machine->machine_name}} @elseif($maintenance->spare && $maintenance->spare->spare_name){{$maintenance->spare->spare_name}} @else Not Available @endif</span>
                            </div>
                            @php
                            if($maintenance->machine && $maintenance->machine->department)
                            {
                                $department = $maintenance->machine->department;
                            } 
                            elseif($maintenance->spare && $maintenance->spare->department)
                            {
                                $department = $maintenance->spare->department;
                            } 
                            @endphp
                            <div style="word-wrap: break-word; margin: 5px 0px;">
                                <span style="font-weight: 500; font-size: 16px; "> Department: @if($department && $department == 'mcl') MCL 
                            ...
                            <div style="word-wrap: break-word; margin: 5px 0px;">
                                <span style="font-weight: 500; font-size: 16px; "> Date: {{$maintenance->created_at->format('d-m-y')}}</span>
                            </div>
                            
                            <div style="word-wrap: break-word; margin: 5px 0px;">
                                <span style="font-weight: 500; font-size: 16px; "> Time: {{$maintenance->created_at->format('h:i A')}}</span>
                            </div>
                        </div>
                    </div>
...
<script>
    function rows_per_page() {
        ...
        window.location.href =
            `/dashboard/maintenance/?${parseResult}&rows_per_page=${per_page}`;
    }
</script>
